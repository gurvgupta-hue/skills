...
The extractor needs tree-sitter and per-language grammar packages. Install them once per session:

```bash
bash setup_env.sh
```

This reports which languages actually got grammars installed. **Trust that report, not your assumptions** — if a language shows as unavailable, files in that language will be skipped (not parsed with degraded accuracy, not regex-guessed) and this must be disclosed in the final documentation, not glossed over.
